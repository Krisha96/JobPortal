...
const JobListing = () => {
    ...
    useEffect(() => {
        ...
        const newFilteredJobs = jobs.slice().reverse().filter(
            job => matchedJobs(job) && matchedLocation(job) && matchesTitle(job) && matchSearchLocation(job)
        );
    ...
    return (
        <div className='container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y-8 py-8'>
            ...
            <div className='w-full lg:w-1/4 bg-white px-4'>
                ...
                <button onClick={e => setshowFilter(prev => !prev)} className='px-6 py-1.5 rounded borer border-gray-400 lg:hidden'>
                    ...
                    <ul className='space-y-4 text-gray-600'>
                        {
                            JobCategories.map((category, index) => (
                                <li className='flex gap-3 items-center' key={index}>
                                    <input className='scale-125' type="checkbox" onChange={() => handleCategoryChange(category)} checked={selectedCategory.includes(category)} />
                                    {category}
